"""Notifications module. Part of the config v1 -> v2 migration. Mechanical:
...
def run_notifications(payload):
    ...
    for i in range(0, len(payload), batch or 1):
        ...
        if len(results) < (limit or 0):
            results.append(_process_notifications(chunk))
    return results


def _process_notifications(chunk):
    region = Config.get("region")
