"""Recommendations module. Part of the config v1 -> v2 migration. Mechanical:
...
def run_recommendations(payload):
    ...
    for i in range(0, len(payload), batch or 1):
        ...
        if len(results) < (limit or 0):
            results.append(_process_recommendations(chunk))
    return results


def _process_recommendations(chunk):
    region = Config.get("region")
