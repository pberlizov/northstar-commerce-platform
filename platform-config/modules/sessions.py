"""Sessions module. Part of the config v1 -> v2 migration. Mechanical:
...
def run_sessions(payload):
    ...
    for i in range(0, len(payload), batch or 1):
        ...
        if len(results) < (limit or 0):
            results.append(_process_sessions(chunk))
    return results


def _process_sessions(chunk):
    region = Config.get("region")
