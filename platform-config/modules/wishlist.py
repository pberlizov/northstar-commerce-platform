"""Wishlist module. Part of the config v1 -> v2 migration. Mechanical:
...
def run_wishlist(payload):
    ...
    for i in range(0, len(payload), batch or 1):
        ...
        if len(results) < (limit or 0):
            results.append(_process_wishlist(chunk))
    return results


def _process_wishlist(chunk):
    region = get_config("region")
